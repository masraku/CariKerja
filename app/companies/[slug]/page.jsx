...
import {
  Building2, MapPin, Users, Briefcase, Globe, Mail, Phone, Star, CheckCircle,
  Calendar, TrendingUp, Award, Facebook, Twitter, Instagram, Linkedin,
  ArrowLeft, ExternalLink, Share2, Flag
} from 'lucide-react'
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50">
        <div className="text-center">
          ...
          <p className="text-gray-600 font-medium">Memuat profil perusahaan...</p>
        </div>
      </div>
    )
  }
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50">
        <div className="text-center max-w-md px-4">
          ...
          <p className="text-gray-600 mb-8">Perusahaan yang Anda cari mungkin telah dihapus atau tautan yang Anda gunakan salah.</p>
          <Link href="/companies" className="inline-flex items-center justify-center px-6 py-3 bg-blue-600 hover:bg-blue-700 text-white font-bold rounded-xl transition-all shadow-lg shadow-blue-600/20">
                ...
                <div className="flex-1 min-w-0 pt-2">
                    ...
                    <div className="flex flex-wrap gap-y-3 gap-x-6 text-sm text-gray-600 mb-6">
                        ...
                        <div className="flex items-center gap-2">
                            ...
                            <span className="font-medium text-blue-600">{company._count.jobs} lowongan aktif</span>
                        </div>
                        {company.averageRatings && (
                            <div className="flex items-center gap-2 px-2 py-0.5 bg-amber-50 rounded-lg border border-amber-100">
                                ...
                                <span className="font-bold text-amber-700">{company.averageRatings.overall}</span>
                                <span className="text-amber-600">({company._count.reviews} reviews)</span>
                            </div>
                        )}
                    </div>
                ...
                <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 lg:p-8 min-h-[400px]">
                                ...
                                <div>
                                    ...
                                    <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                                        {company.gallery.map((image, index) => (
                                            <div key={index} className="aspect-video rounded-xl overflow-hidden shadow-sm border border-gray-100 group">
                                                <img 
                                                    src={image} 
                                                    alt={`Gallery ${index + 1}`} 
                    ...
                    {activeTab === 'jobs' && (
                        <div className="space-y-4">
                            <h2 className="text-xl font-bold text-gray-900 mb-6">Lowongan Kerja Aktif</h2>
                            {company.jobs.length === 0 ? (
                                <div className="text-center py-12 bg-gray-50 rounded-2xl border border-dashed border-gray-200">
                                    <Briefcase className="w-12 h-12 text-gray-300 mx-auto mb-3" />
                                    <p className="text-gray-500 font-medium">Belum ada lowongan aktif saat ini</p>
                                </div>
                            ) : (
                                    ...
                                    >
                                        ...
                                        <div className="flex items-center justify-between pt-4 border-t border-gray-50 mt-4">
                                            {job.showSalary && job.salaryMin && job.salaryMax ? (
                                                ...
                                                <p className="text-blue-600 font-bold text-sm">Gaji Kompetitif</p>
                                            )}
                    ...
                    {activeTab === 'benefits' && (
                        <div>
                            <h2 className="text-xl font-bold text-gray-900 mb-6">Benefits & Fasilitas</h2>
                            {company.benefits?.length > 0 ? (
                                <div className="grid md:grid-cols-2 gap-4">
                                    {company.benefits.map((benefit, index) => (
                                        <div key={index} className="flex items-start gap-3 p-4 bg-green-50/50 border border-green-100 rounded-xl">
                                ...
                                <div className="text-center py-12 bg-gray-50 rounded-2xl border border-dashed border-gray-200">
                                    ...
                                    <p className="text-gray-500 font-medium">Informasi benefits belum tersedia</p>
                                </div>
                            )}
                        </div>
                    )}
                    ...
                    {activeTab === 'reviews' && (
                        <div>
                            <div className="flex items-center justify-between mb-6">
                                ...
                                <div className="flex items-center gap-2">
                                    <span className="text-3xl font-bold text-gray-900">{company.averageRatings?.overall || 0}</span>
                                    <div className="flex flex-col">
                                        <div className="flex">
                                            {[...Array(5)].map((_, i) => (
                                                <Star key={i} className={`w-4 h-4 ${i < Math.floor(company.averageRatings?.overall || 0) ? 'text-amber-400 fill-amber-400' : 'text-gray-200'}`} />
                                            ))}
                                        </div>
                                        <span className="text-xs text-gray-500">{company._count.reviews} reviews</span>
                                    </div>
                                </div>
                            </div>
                            ...
                            {company.reviews.length === 0 ? (
                                <div className="text-center py-12 bg-gray-50 rounded-2xl border border-dashed border-gray-200">
                                    ...
                                    <p className="text-gray-500 font-medium">Belum ada review untuk perusahaan ini</p>
                                </div>
                            ) : (
                                <div className="space-y-6">
                                    {company.reviews.map((review) => (
                                        <div key={review.id} className="p-6 border border-gray-100 rounded-2xl bg-gray-50/50">
                                                ...
                                                <div className="flex-1">
                                                    <div className="flex items-center justify-between mb-1">
                                                        <h4 className="font-bold text-gray-900">
                                                            {review.isAnonymous ? 'Anonymous User' : review.jobseeker?.firstName}
                                                        </h4>
                                            ...
                                            <h5 className="font-bold text-gray-900 mb-2 text-lg">{review.title}</h5>
            ...
            <div className="space-y-6">
                ...
                <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
                    ...
                    <div className="space-y-5 text-sm">
                        <div>
                            <p className="text-gray-500 mb-1 text-xs uppercase tracking-wider font-medium">Industri</p>
                            <p className="font-semibold text-gray-900 text-base">{company.industry}</p>
                        </div>

                        <div>
                            <p className="text-gray-500 mb-1 text-xs uppercase tracking-wider font-medium">Ukuran Perusahaan</p>
                            <p className="font-semibold text-gray-900 text-base">{company.companySize} karyawan</p>
                        </div>

                        {company.foundedYear && (
                            <div>
                                <p className="text-gray-500 mb-1 text-xs uppercase tracking-wider font-medium">Tahun Didirikan</p>
                                <p className="font-semibold text-gray-900 text-base">{company.foundedYear}</p>
                            </div>
                        )}

                        <div>
                            <p className="text-gray-500 mb-1 text-xs uppercase tracking-wider font-medium">Lokasi Kantor Pusat</p>
                        ...
                        {company.email && (
                            <div>
                                <p className="text-gray-500 mb-1 text-xs uppercase tracking-wider font-medium">Email Kontak</p>
                                <a href={`mailto:${company.email}`} className="font-semibold text-blue-600 hover:text-blue-700 flex items-center gap-2 transition-colors">
                        ...
                        {company.phone && (
                            <div>
                                <p className="text-gray-500 mb-1 text-xs uppercase tracking-wider font-medium">Nomor Telepon</p>
                                <a href={`tel:${company.phone}`} className="font-semibold text-blue-600 hover:text-blue-700 flex items-center gap-2 transition-colors">
