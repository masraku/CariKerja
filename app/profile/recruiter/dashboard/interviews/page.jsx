...
export default function RecruiterInterviewsPage() {
    ...
    const handleApproveReschedule = (interview, participant) => {
        router.push(`/profile/recruiter/dashboard/interview/reschedule?interviewId=${interview.id}&participantId=${participant.id}`)
    }
    ...
    const handleRejectReschedule = async (interview, participant) => {
        ...
        try {
            ...
            const response = await fetch(`/api/profile/recruiter/interviews/${interview.id}/participants/${participant.id}`, {
                method: 'PATCH',
                headers: {
                    'Authorization': `Bearer ${token}`,
    ...
    const handleDeleteInterview = async (interview) => {
        ...
        try {
            ...
            const response = await fetch(`/api/profile/recruiter/interviews/${interview.id}`, {
                method: 'DELETE',
                headers: {
                    'Authorization': `Bearer ${token}`
                }
    ...
    const handleMarkComplete = async (interview) => {
        ...
        try {
            ...
            const response = await fetch(`/api/profile/recruiter/interviews/${interview.id}/complete`, {
                method: 'PATCH',
                headers: {
                    'Authorization': `Bearer ${token}`
                }
    ...
    const getFilteredInterviews = () => {
        ...
        if (activeFilter === 'scheduled') return interviews.filter(i => i.status === 'SCHEDULED')
        if (activeFilter === 'reschedule') return interviews.filter(i => 
            i.participants?.some(p => p.status === 'RESCHEDULE_REQUESTED')
        )
        if (activeFilter === 'completed') return interviews.filter(i => i.status === 'COMPLETED')
        if (activeFilter === 'cancelled') return interviews.filter(i => i.status === 'CANCELLED')
        return interviews
    }
    ...
    const rescheduleCount = interviews.reduce((count, interview) => {
        return count + (interview.participants?.filter(p => p.status === 'RESCHEDULE_REQUESTED').length || 0)
    }, 0)
        ...
        <div className="min-h-screen bg-gray-50 py-8">
            <div className="container mx-auto px-4 max-w-6xl">
                ...
                <div className="mb-6">
                    ...
                    <h1 className="text-3xl font-bold text-gray-900 mb-2">Kelola Interview</h1>
                    <p className="text-gray-600">Kelola jadwal interview dan respon kandidat</p>
                </div>
                ...
                <div className="grid grid-cols-2 md:grid-cols-5 gap-4 mb-8">
                    ...
                    >
                        <div className="flex items-center justify-between">
                            <div className="text-left">
                                <p className={`text-xs font-medium ${activeFilter === 'scheduled' ? 'text-green-100' : 'text-green-700'}`}>Terjadwal</p>
                                <p className={`text-2xl font-bold ${activeFilter === 'scheduled' ? 'text-white' : 'text-green-900'}`}>{stats?.scheduled || 0}</p>
                            </div>
                            <CheckCircle className={`w-8 h-8 ${activeFilter === 'scheduled' ? 'text-green-200' : 'text-green-600'}`} />
                        </div>
                    </button>
                    ...
                    >
                        ...
                        <div className="flex items-center justify-between">
                            <div className="text-left">
                                ...
                                <p className={`text-2xl font-bold ${activeFilter === 'reschedule' ? 'text-white' : 'text-orange-900'}`}>{rescheduleCount}</p>
                            </div>
                    ...
                    >
                        <div className="flex items-center justify-between">
                            <div className="text-left">
                                <p className={`text-xs font-medium ${activeFilter === 'completed' ? 'text-purple-100' : 'text-purple-700'}`}>Selesai</p>
                                <p className={`text-2xl font-bold ${activeFilter === 'completed' ? 'text-white' : 'text-purple-900'}`}>{stats?.completed || 0}</p>
                            </div>
                            <Check className={`w-8 h-8 ${activeFilter === 'completed' ? 'text-purple-200' : 'text-purple-600'}`} />
                        </div>
                    </button>
                    ...
                    >
                        <div className="flex items-center justify-between">
                            <div className="text-left">
                                <p className={`text-xs font-medium ${activeFilter === 'cancelled' ? 'text-gray-300' : 'text-gray-600'}`}>Dibatalkan</p>
                                <p className="text-2xl font-bold">{stats?.cancelled || 0}</p>
                            </div>
                            <XCircle className={`w-8 h-8 ${activeFilter === 'cancelled' ? 'text-gray-400' : 'text-gray-400'}`} />
                        </div>
                    </button>
                </div>
                ...
                <div className="space-y-4">
                    {filteredInterviews.map((interview) => {
                        const rescheduleParticipants = interview.participants?.filter(p => p.status === 'RESCHEDULE_REQUESTED') || []
                        ...
                        return (
                            ...
                            >
                                    ...
                                    <div className="bg-orange-50 border border-orange-200 rounded-xl p-4 mb-4">
                                        ...
                                        <div className="space-y-3">
                                            {rescheduleParticipants.map((participant) => (
                                                <div key={participant.id} className="bg-white rounded-lg p-4 border border-orange-200">
                                                    <div className="flex items-start justify-between">
                                                        <div className="flex items-center gap-3">
                                                            <div className="w-10 h-10 bg-orange-100 rounded-full flex items-center justify-center">
                                                                {participant.jobseeker?.photo ? (
                                                                    <img src={participant.jobseeker.photo} alt="" className="w-10 h-10 rounded-full object-cover" />
                                                                ) : (
                                                                    <span className="text-orange-600 font-bold">
                                                                        {participant.jobseeker?.firstName?.charAt(0) || '?'}
                                                                    </span>
                                                                )}
                                                            </div>
                                                            <div>
                                                                <p className="font-semibold text-gray-900">
                                                                    {participant.jobseeker?.firstName} {participant.jobseeker?.lastName}
                                                                </p>
                                                                <p className="text-sm text-gray-500">
                                                                    {participant.jobseeker?.currentTitle || 'Kandidat'}
                                                                </p>
                                                            </div>
                                                        </div>
                                                        <div className="flex gap-2">
                                                            <button
                                                                onClick={() => handleApproveReschedule(interview, participant)}
                                                            ...
                                                            <button
                                                                onClick={() => handleRejectReschedule(interview, participant)}
                                                    ...
                                                    {participant.responseMessage && (
                                                        <div className="mt-3 p-3 bg-orange-50 rounded-lg">
                                                            <p className="text-sm text-orange-800">
                                                                <strong>Alasan:</strong> {participant.responseMessage}
                                                            </p>
                                                        </div>
                                                    )}
                                                </div>
                                ...
                                <div className="space-y-2">
                                    <h4 className="font-semibold text-gray-700 mb-2">Kandidat:</h4>
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                                        {interview.participants?.filter(p => p.status !== 'RESCHEDULE_REQUESTED').map((participant) => (
                                            <div key={participant.id} className="flex items-center justify-between bg-gray-50 rounded-lg p-3">
                                                <div className="flex items-center gap-2">
                                                    <div className="w-8 h-8 bg-gray-200 rounded-full flex items-center justify-center">
                                                        {participant.jobseeker?.photo ? (
                                                            <img src={participant.jobseeker.photo} alt="" className="w-8 h-8 rounded-full object-cover" />
                                                        ) : (
                                                ...
                                                <span className={`text-xs px-2 py-1 rounded-full ${
                                                    ...
                                                    {participant.status === 'ACCEPTED' ? 'Hadir' :
                                                     participant.status === 'DECLINED' ? 'Tidak Hadir' :
                                                     'Pending'}
                                                </span>
                                            </div>
                                        ))}
                ...
                {filteredInterviews.length === 0 && (
                    <div className="bg-white rounded-2xl shadow-lg p-12 text-center">
                        ...
                        <h3 className="text-xl font-bold text-gray-900 mb-2">Tidak Ada Interview</h3>
