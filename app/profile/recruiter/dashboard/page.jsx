...
export default function RecruiterDashboard() {
  ...
  const filteredApplications = activeFilter === 'ALL' 
    ? recentApplications 
    : recentApplications.filter(app => { // This filters the *recent* list. Ideally should filter all or fetch filtered. For now, filter client side on recent list is OK or clarification needed.
        // But mapped to stats:
        if (activeFilter === 'PENDING') return ['PENDING', 'REVIEWING'].includes(app.status)
    ...
    <div className="min-h-screen bg-gray-50">
        ...
        <div className="mb-8">
          ...
          <p className="text-gray-600">
            Hai, <span className="font-semibold text-blue-600">{company.name}</span>! Kelola lowongan dan pelamar Anda
          </p>
        </div>
           ...
           <div className="mt-6 pt-6 border-t border-gray-100">
              {company.status === 'REJECTED' && (
                <div className="p-4 bg-red-50 border border-red-200 rounded-xl flex items-start gap-3">
                   ...
                   <div>
                      ...
                      <p className="text-red-700 text-sm mt-1">{company.rejectionReason || 'Mohon perbaiki data perusahaan Anda.'}</p>
                   </div>
                </div>
              )}
              {!company.verified && company.status !== 'REJECTED' && (
                 <div className="p-4 bg-amber-50 border border-amber-200 rounded-xl flex items-start gap-3">
                   ...
                   <div>
                      <h4 className="font-bold text-amber-900">Menunggu Verifikasi</h4>
                      <p className="text-amber-700 text-sm mt-1">Tim kami sedang meninjau dokumen legalitas perusahaan Anda.</p>
                   </div>
                </div>
              )}
              {company.verified && (
                 <div className="p-4 bg-green-50 border border-green-200 rounded-xl flex items-start gap-3">
                   ...
                   <div>
                      <h4 className="font-bold text-green-900">Akun Terverifikasi</h4>
                      <p className="text-green-700 text-sm mt-1">Anda memiliki akses penuh ke fitur premium.</p>
                   </div>
                </div>
              )}
           </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
          <div 
             ...
             className={`bg-white rounded-2xl p-6 border transition-all cursor-pointer hover:shadow-md ${activeFilter === 'ALL' ? 'ring-2 ring-blue-500 border-transparent shadow-md' : 'border-gray-100 shadow-sm'}`}
          >
            <div className="flex items-center justify-between mb-4">
              <div className="w-12 h-12 bg-blue-50 rounded-xl flex items-center justify-center">
                <Briefcase className="w-6 h-6 text-blue-600" />
              </div>
              <span className={`text-xs font-bold px-2 py-1 rounded-full ${activeFilter === 'ALL' ? 'bg-blue-100 text-blue-700' : 'bg-gray-100 text-gray-500'}`}>
                Total
              </span>
            </div>
          ...
          <div 
             ...
             className={`bg-white rounded-2xl p-6 border transition-all cursor-pointer hover:shadow-md ${activeFilter === 'PENDING' ? 'ring-2 ring-yellow-500 border-transparent shadow-md' : 'border-gray-100 shadow-sm'}`}
          >
             <div className="flex items-center justify-between mb-4">
              <div className="w-12 h-12 bg-yellow-50 rounded-xl flex items-center justify-center">
                <Clock className="w-6 h-6 text-yellow-600" />
              </div>
               {activeFilter === 'PENDING' && <CheckCircle className="w-4 h-4 text-yellow-600" />}
            </div>
            <p className="text-3xl font-bold text-gray-900 mb-1">{stats.pendingApplications + stats.reviewingApplications}</p>
            <p className="text-sm text-gray-500">Perlu Direview</p>
          </div>

          <div 
             onClick={() => handleStatClick('SHORTLISTED')}
             className={`bg-white rounded-2xl p-6 border transition-all cursor-pointer hover:shadow-md ${activeFilter === 'SHORTLISTED' ? 'ring-2 ring-purple-500 border-transparent shadow-md' : 'border-gray-100 shadow-sm'}`}
          >
             <div className="flex items-center justify-between mb-4">
              <div className="w-12 h-12 bg-purple-50 rounded-xl flex items-center justify-center">
                <TrendingUp className="w-6 h-6 text-purple-600" />
              </div>
              {activeFilter === 'SHORTLISTED' && <CheckCircle className="w-4 h-4 text-purple-600" />}
            </div>
            <p className="text-3xl font-bold text-gray-900 mb-1">{stats.shortlistedApplications}</p>
            ...
            <div className="lg:col-span-2 space-y-8">
                 ...
                 <div className="grid sm:grid-cols-3 gap-4">
                     <button
                        ...
                        disabled={company.status === 'REJECTED' || (!company.verified && company.status !== 'PENDING')}
                     ...
                     >
                        ...
                        <p className="text-sm text-gray-500 mt-1">Posting pekerjaan baru</p>
                     </button>
                     ...
                     >
                        ...
                        <p className="text-sm text-gray-500 mt-1">Lihat semua lowongan</p>
                     </button>
                     ...
                     >
                        ...
                        <p className="text-sm text-gray-500 mt-1">Review lamaran masuk</p>
                     </button>
                 </div>
                 ...
                 <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
                    <div className="flex items-center justify-between mb-6">
                        <h3 className="font-bold text-lg text-gray-900">Lowongan Terbaru</h3>
                        <button onClick={() => router.push('/profile/recruiter/dashboard/jobs')} className="text-sm text-blue-600 font-medium hover:underline">Lihat Semua</button>
                    </div>
                    
                    {company.jobs.length > 0 ? (
                        <div className="space-y-4">
                            {company.jobs.slice(0, 3).map(job => (
                                <div key={job.id} onClick={() => router.push(`/profile/recruiter/dashboard/jobs/${job.slug}`)} className="flex items-center justify-between p-4 rounded-xl border border-gray-100 hover:border-blue-200 hover:bg-blue-50/30 transition-all cursor-pointer group">
                                    <div>
                                    ...
                                    <div className="text-right">
                                         <div className="text-2xl font-bold text-gray-900 group-hover:text-blue-700 transition-colors">{job._count.applications}</div>
                        ...
                        <div className="text-center py-8 text-gray-500">Belum ada lowongan aktif</div>
                    )}
                 </div>
            </div>
                 ...
                 <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 h-full">
                    <div className="flex items-center justify-between mb-6">
                        <h3 className="font-bold text-lg text-gray-900">
                            {activeFilter === 'ALL' ? 'Lamaran Terbaru' : `Lamaran ${activeFilter === 'PENDING' ? 'Pending' : 'Shortlisted'}`}
                        </h3>
                         <button onClick={() => router.push('/profile/recruiter/dashboard/applications')} className="text-sm text-blue-600 font-medium hover:underline">Lihat Semua</button>
                    </div>
                    ...
                    {filteredApplications.length > 0 ? (
                                    ...
                                    <div className="flex items-center gap-3 mb-3">
                                        <div className="w-10 h-10 rounded-full bg-gray-200 overflow-hidden">
                                             {app.jobseekers.photo ? (
                                                <img src={app.jobseekers.photo} className="w-full h-full object-cover" />
                                             ) : (
                                                <div className="w-full h-full flex items-center justify-center text-gray-500 font-bold">{app.jobseekers.firstName?.charAt(0)}</div>
                                             )}
                                        </div>
                                        <div>
                                            <div className="font-bold text-gray-900 text-sm">{app.jobseekers.firstName} {app.jobseekers.lastName}</div>
                                            <div className="text-xs text-gray-500">{app.jobs.title}</div>
                                        </div>
                                    </div>
                                    <div className="flex items-center justify-between">
                                         <span className={`text-[10px] font-bold px-2 py-1 rounded-full ${getStatusBadge(app.status).color}`}>
                        ...
                        <div className="text-center py-12">
                             ...
                             <p className="text-gray-500 text-sm">Tidak ada lamaran {activeFilter !== 'ALL' && 'untuk filter ini'}</p>
                        </div>
                    )}
                 </div>
            </div>
